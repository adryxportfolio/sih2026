...
until "${compose[@]}" exec -T postgres pg_isready -U samiksha >/dev/null 2>&1; do
  sleep 1
done
...
"${compose[@]}" exec -T postgres psql -X -U samiksha -d samiksha \
  --set=ON_ERROR_STOP=on --single-transaction --file=- < "$SRC/samiksha.sql"
