import type { ProcessEvent } from "@samiksha/adapter-kit";
...
describe("Docker sandbox", () => {
    ...
    expect(fetchMock).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        ...
        headers: expect.objectContaining({
          ...
          "x-samiksha-bot-id": "home-bot",
          "x-samiksha-screen-id": "bot",
          "x-samiksha-screen-lease-id": "run-1:1",
  ...
  it("asks the supervisor to cancel orphaned run work when releasing a screen", async () => {
    ...
    expect(fetchMock).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        method: "DELETE",
        headers: expect.objectContaining({ "x-samiksha-cancel-run-work": "1" }),
      }),
    );
  });
});
