import type { AdapterContext } from "@samiksha/adapter-kit";
...
describe("SerenityMemoryProvider", () => {
  it("keeps bot and space entity namespaces inside the adapter", () => {
    expect(serenityBotEntity("bot-1")).toBe("samiksha-bot/bot-1");
    expect(serenitySpaceEntity("workspace-1")).toBe("samiksha-space/workspace-1");
    expect(serenityBotEntity("bot-1", "Personal Brain")).toBe(`samiksha-bot/${PERSONAL_BRAIN}/bot-1`);
    expect(serenitySpaceEntity("workspace-1", "Personal Brain")).toBe(
      `samiksha-space/${PERSONAL_BRAIN}/workspace-1`,
    );
  });
  ...
  it("recalls isolated scope only against the bot entity", async () => {
    ...
    expect(recallSerenityMock).toHaveBeenCalledWith(
      ...
      expect.objectContaining({ entity: "samiksha-bot/bot-1", limit: 5 }),
    );
  });
  ...
  it("scopes shared durable saves by brain label when configured", async () => {
    ...
    expect(rememberSerenityMock.mock.calls.map((call) => call[3]?.entity)).toEqual([
      `samiksha-space/${PERSONAL_BRAIN}/workspace-1`,
      `samiksha-bot/${PERSONAL_BRAIN}/bot-1`,
    ]);
  });
  ...
  it("forgets by fact id only (Serenity forget has no entity argument)", async () => {
    ...
    await labeled.forget(
      { id: "fact-9", entity: `samiksha-bot/${PERSONAL_BRAIN}/bot-1`, reason: "cleanup" },
      context,
    );
      ...
      value: [
        {
          ...
          id: "fact-space-1",
          provenance: "space policy",
          entity: `samiksha-space/${PERSONAL_BRAIN}/workspace-1`,
        },
      ],
    });
