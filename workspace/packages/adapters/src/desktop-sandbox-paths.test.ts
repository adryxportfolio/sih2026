...
describe("desktop sandbox path rules", () => {
  it("compares Windows roots case-insensitively without accepting siblings or other drives", () => {
    const roots = ["C:\\Users\\Owner\\Samiksha\\bot"];

    expect(
      isAllowedDesktopPath("c:\\users\\owner\\samiksha\\BOT\\notes.txt", roots, path.win32),
    ).toBe(true);
    expect(
      isAllowedDesktopPath("C:\\Users\\Owner\\Samiksha\\bot-other\\notes.txt", roots, path.win32),
    ).toBe(false);
  ...
  it("handles explicitly allowed UNC roots without accepting sibling shares", () => {
    const roots = ["\\\\server\\share\\samiksha\\bot"];

    expect(
      isAllowedDesktopPath("\\\\SERVER\\share\\samiksha\\BOT\\notes.txt", roots, path.win32),
    ).toBe(true);
    expect(
      isAllowedDesktopPath("\\\\server\\share-other\\samiksha\\bot\\notes.txt", roots, path.win32),
    ).toBe(false);
  });
