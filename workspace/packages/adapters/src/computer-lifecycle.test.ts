...
import { clearThread, type PrismaClient, type ThreadEvents } from "@samiksha/db";
...
describe("computer provisioning", () => {
  it("stops a provider when archive invalidates its boot claim", async () => {
    const dataDir = await mkdtemp(path.join(tmpdir(), "samiksha-provision-race-"));
  ...
  it("does not adopt a concurrent reclaim stamp after claiming", async () => {
    const dataDir = await mkdtemp(path.join(tmpdir(), "samiksha-claim-stamp-race-"));
    const row = {
      id: "computer-1",
      homeKey: "bot-1",
      providerRef: null as string | null,
  ...
  it("advances the claim stamp past the observed updatedAt when the clock does not move", async () => {
    const dataDir = await mkdtemp(path.join(tmpdir(), "samiksha-claim-stamp-skew-"));
  ...
  it("does not reclaim a fresh booting claim without waiting", async () => {
    const dataDir = await mkdtemp(path.join(tmpdir(), "samiksha-fresh-boot-claim-"));
  ...
  it("does not reclaim a stale booting claim while another run still holds a live worker lease", async () => {
    const dataDir = await mkdtemp(path.join(tmpdir(), "samiksha-stale-boot-live-run-"));
    const row = {
      id: "computer-1",
      homeKey: "bot-1",
      providerRef: null as string | null,
  ...
  it("activates a boot even if another Team bot takes a lease mid-provision", async () => {
    const dataDir = await mkdtemp(path.join(tmpdir(), "samiksha-team-lease-mid-boot-"));
    const row = {
      id: "computer-1",
      homeKey: "bot-1",
      providerRef: null as string | null,
  ...
  it.each([
    ...
    async ({ stage, rollbackFails }) => {
      const dataDir = await mkdtemp(path.join(tmpdir(), "samiksha-reconnect-rollback-"));
  ...
  it("does not stop a pre-existing computer when reconnect setup fails", async () => {
    const dataDir = await mkdtemp(path.join(tmpdir(), "samiksha-reconnect-unowned-"));
  ...
  it.each([
    ...
    async (next) => {
      const dataDir = await mkdtemp(path.join(tmpdir(), "samiksha-provision-reconnect-update-"));
  ...
  it.each([
    ...
    const dataDir = await mkdtemp(path.join(tmpdir(), "samiksha-prepare-rollback-"));
    const ref = {
      id: "provider-1",
      botId: "bot-1",
      kind: "fake" as const,
  ...
  it("releases the screen when activation fails on a resumed Team computer", async () => {
    const dataDir = await mkdtemp(path.join(tmpdir(), "samiksha-team-activation-rollback-"));
    const ref = {
      id: "provider-1",
      botId: "team-home",
  ...
  it("retains a fresh provider reference when rollback also fails", async () => {
    const dataDir = await mkdtemp(path.join(tmpdir(), "samiksha-prepare-rollback-failure-"));
  ...
  it.each([false, true])(
    ...
    async (prepareFails) => {
      const dataDir = await mkdtemp(path.join(tmpdir(), "samiksha-provision-reconnect-"));
      const ref = {
        id: "provider-1",
        botId: "bot-1",
        kind: "cloud" as const,
...
describe("computer replacement", () => {
  ...
  it("replaces a wedged computer and restores the durable home", async () => {
    const dataDir = await mkdtemp(path.join(tmpdir(), "samiksha-replace-"));
    const homeRoot = await mkdtemp(path.join(tmpdir(), "samiksha-replace-home-"));
  ...
  it("claims a stopped computer before teardown so concurrent replacements serialize", async () => {
    const dataDir = await mkdtemp(path.join(tmpdir(), "samiksha-replace-stopped-claim-"));
    const updateMany = vi
      .fn()
      .mockResolvedValueOnce({ count: 1 })
      .mockResolvedValueOnce({ count: 1 })
  ...
  it("continues recover when checkpoint fails with an ordinary provider error", async () => {
    const dataDir = await mkdtemp(path.join(tmpdir(), "samiksha-recover-checkpoint-"));
    const homeRoot = await mkdtemp(path.join(tmpdir(), "samiksha-recover-checkpoint-home-"));
    const home = new LocalAgentHomeStore(homeRoot);
    const sandbox = new FakeSandboxProvider();
    const first = await sandbox.provision({ botId: "bot-1", homePath: dataDir }, context);
  ...
  it("aborts update when checkpoint fails with an ordinary provider error", async () => {
    const dataDir = await mkdtemp(path.join(tmpdir(), "samiksha-update-checkpoint-"));
    const homeRoot = await mkdtemp(path.join(tmpdir(), "samiksha-update-checkpoint-home-"));
    const home = new LocalAgentHomeStore(homeRoot);
    const sandbox = new FakeSandboxProvider();
    const first = await sandbox.provision({ botId: "bot-1", homePath: dataDir }, context);
    vi.spyOn(sandbox, "exportWorkspace").mockImplementation(() => {
