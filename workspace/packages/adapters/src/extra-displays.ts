import type { ComputerAction, ComputerInput } from "@samiksha/adapter-kit";
...
export function observeExtraDisplayCommand(layout: ExtraDisplayLayout): string {
  const imagePath = `/tmp/samiksha/observe-${layout.displayNumber}.png`;
  return [
    `DISPLAY=${layout.display} xdotool getmouselocation --shell >/tmp/samiksha/cursor-${layout.displayNumber}.txt || true`,
    ...
    `printf '\\nCURSOR '`,
    `tr '\\n' ' ' </tmp/samiksha/cursor-${layout.displayNumber}.txt 2>/dev/null || true`,
  ].join("; ");
}
