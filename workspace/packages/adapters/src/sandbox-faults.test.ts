...
import type { PortableFile, SandboxProvider } from "@samiksha/adapter-kit";
...
describe.each([
  ...
  [
    "desktop",
    async () => {
      const root = await mkdtemp(path.join(tmpdir(), "samiksha-sandbox-faults-"));
...
async function providerSet(label: string): Promise<Array<[string, SandboxProvider]>> {
  const root = await mkdtemp(path.join(tmpdir(), `samiksha-sandbox-transfer-${label}-`));
  temporaryRoots.push(root);
  return [
