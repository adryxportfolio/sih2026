...
import type { createRepos, PrismaClient } from "@samiksha/db";
...
describe("spawned bot creation", () => {
  ...
  it("passes computerMode through to createBot", async () => {
    ...
    const createReposSpy = vi.spyOn(await import("@samiksha/db"), "createRepos").mockReturnValue({
      createBot,
      ...
      destroyBot(
        {
          ...
          home: {} as AgentHomeStore,
          jobs: { cancel: vi.fn() } as unknown as JobPublisher,
          dataDir: "/tmp/samiksha-destroy-bot-test",
        },
        {
          id: "bot-1",
          userId: "user-1",
