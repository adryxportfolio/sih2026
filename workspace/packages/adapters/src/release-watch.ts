...
/** Seeded samiksha releases so evals succeed without the public internet. */
export const DEFAULT_SAMIKSHA_EMULATED_RELEASES: readonly EmulatedGithubRelease[] = [
  {
    owner: "elie222",
    repo: "samiksha",
    tag: "v0.4.2",
    ...
    htmlUrl: "https://github.com/elie222/samiksha/releases/tag/v0.4.2",
  },
  {
    owner: "elie222",
    repo: "samiksha",
    ...
    htmlUrl: "https://github.com/elie222/samiksha/releases/tag/v0.4.1",
  },
];
...
export function assessReleaseWatchRoutinePrompt(prompt: string): {
  ...
  const mentionsRepo = /samiksha|elie222/i.test(text);
  ...
  if (!mentionsRepo || !mentionsReleases || !mentionsGithubTool) {
    const missing = [
      !mentionsRepo ? "repo (elie222/samiksha)" : null,
