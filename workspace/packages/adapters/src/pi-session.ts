 ...
 * This is deliberately a recorder only: Samiksha remains responsible for running
...
export class PiJsonlSessionRecorder implements PiSessionRecorder {
  ...
  async start(input: PiSessionStart): Promise<PiSessionHandle> {
    ...
    await handle.appendCustomEntry("samiksha_context", {
      samikshaUserId: input.userId,
      samikshaBotId: input.botId,
      samikshaRunId: input.runId,
      samikshaThreadId: input.threadId,
      ...(input.traceId ? { samikshaTraceId: input.traceId } : {}),
      model: input.model,
