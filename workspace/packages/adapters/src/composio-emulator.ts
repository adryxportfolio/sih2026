...
import {
  DEFAULT_SAMIKSHA_EMULATED_RELEASES,
    ...
    messages: [
      {
        ...
        subject: "Welcome to Samiksha",
        sender: "hello@samiksha.test",
        to: "me@example.test",
  ...
  return [
    {
      ...
      inputSchema: {
        ...
        properties: {
          ...
          repo: { type: "string", description: "Repository name, e.g. samiksha" },
        },
        required: ["owner", "repo"],
      },
    },
...
export class ComposioEmulator implements ComposioProvider {
  ...
  private githubReleases: EmulatedGithubRelease[] = [...DEFAULT_SAMIKSHA_EMULATED_RELEASES];
  ...
  private executeGithub(tool: string, args: Record<string, unknown>): Record<string, unknown> {
    ...
    const repo = String(args.repo ?? args.repository ?? "samiksha");
