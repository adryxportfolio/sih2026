...
export function serenityBotEntity(botId: string, brainLabel = ""): string {
  ...
  return label ? `samiksha-bot/${label}/${botId}` : `samiksha-bot/${botId}`;
}

export function serenitySpaceEntity(spaceId: string, brainLabel = ""): string {
  const label = sanitizeSerenityBrainLabel(brainLabel);
  return label ? `samiksha-space/${label}/${spaceId}` : `samiksha-space/${spaceId}`;
}
...
export class SerenityMemoryProvider implements SemanticMemoryProvider {
  ...
  async recall(
    ...
    // History compaction stays in Samiksha; Serenity is the durable brain only.
  ...
  async save(
    ...
    const provenance = `samiksha space:${context.spaceId} bot:${request.botId}`;
    const results = await Promise.all(
      entities.map((entity) =>
