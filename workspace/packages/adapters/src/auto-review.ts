import type { AgentModelOAuthCredential, AgentRuntime } from "@samiksha/adapter-kit";
import type { ActionApprovalRule } from "@samiksha/core";
import { type AutoReviewJudgeDecision, redactSecrets } from "@samiksha/core";
...
function localModelIds(env: NodeJS.ProcessEnv): string[] {
  return (env.SAMIKSHA_LOCAL_MODELS ?? "")
    .split(",")
    .map((id) => id.trim())
...
export function deploymentAutoReviewDefault(env: NodeJS.ProcessEnv = process.env): boolean {
  return envFlag(env, "SAMIKSHA_AUTO_REVIEW");
}

export function autoReviewTimeoutMs(env: NodeJS.ProcessEnv = process.env): number {
  const raw = env.SAMIKSHA_AUTO_REVIEW_TIMEOUT_MS?.trim();
...
export function resolveAutoReviewChecker(
  ...
  const overrideProvider = env.SAMIKSHA_AUTO_REVIEW_PROVIDER?.trim();
  const overrideModel = env.SAMIKSHA_AUTO_REVIEW_MODEL?.trim();
