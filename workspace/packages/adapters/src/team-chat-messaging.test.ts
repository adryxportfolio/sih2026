import type { MessagingInboundMessage, MessagingSurface } from "@samiksha/adapter-kit";
...
describe("toTeamChatInbound", () => {
  ...
  it("treats @mentions as mention when kind is omitted", () => {
    expect(toTeamChatInbound(baseEvent({ content: "hey @samiksha look" }))?.kind).toBe("mention");
  });
});
