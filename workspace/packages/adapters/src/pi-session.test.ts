...
describe("Pi JSONL sessions", () => {
  ...
  it("uses Pi's session format for context and completed messages", async () => {
    const root = await mkdtemp(path.join(os.tmpdir(), "samiksha-pi-session-"));
    try {
      ...
      expect(records).toContainEqual(
        expect.objectContaining({
          kind: "entry",
          customType: "samiksha_context",
          data: expect.objectContaining({
            samikshaThreadId: "thread-1",
            samikshaTraceId: "trace-1",
          }),
        }),
      );
  ...
  it("keeps sessions scoped and bounded by the retention policy", async () => {
    const root = await mkdtemp(path.join(os.tmpdir(), "samiksha-pi-retention-"));
    try {
  ...
  it("removes bot and account session scopes", async () => {
    const root = await mkdtemp(path.join(os.tmpdir(), "samiksha-pi-cleanup-"));
    try {
