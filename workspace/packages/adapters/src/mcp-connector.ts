...
import { isLocalMcpHost } from "@samiksha/contracts";
import type { McpServer, PrismaClient } from "@samiksha/db";
...
export class McpConnector implements ConnectorProvider {
  ...
  private async connectSession(server: McpServer, context: AdapterContext): Promise<McpSession> {
    const session = new McpSession({ name: `samiksha-${server.slug}` });
    try {
