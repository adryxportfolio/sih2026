import type { BackgroundJobHandlers } from "@samiksha/adapter-kit";
...
describe("InMemoryJobQueue", () => {
  ...
  it("keeps wrapped payloads readable by workers that do not unwrap envelopes", async () => {
    const { wrapJobPayload } = await import("@samiksha/logging");
    const { parseBackgroundJob } = await import("@samiksha/adapter-kit");
