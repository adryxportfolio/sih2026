import type { AdapterContext, ConnectorEvent, ConnectorTool } from "@samiksha/adapter-kit";
import { createLogger, createTestSink, installLogger } from "@samiksha/logging";
...
describe("composio tool mapping", () => {
  ...
  it("logs sanitized connector discovery failures", async () => {
    ...
    installLogger(createLogger({ service: "samiksha-api", sinks: [sink] }));
    ...
    try {
      ...
      installLogger(createLogger({ service: "samiksha-api", level: "off", sinks: [] }));
    }
  });
  ...
  it("uses catalog-canonical toolkit slugs without preloading every tool", async () => {
    ...
    expect(
      ...
      { userId: "__samiksha_catalog__", toolkits: undefined, sessionPreset: undefined },
  ...
  it("only fetches live Composio slugs when a Samiksha row is still pending or errored", () => {
  ...
  it("plans DB sync when Composio is connected but Samiksha is still pending", () => {
    expect(
      planLiveConnectionSync(
        [
