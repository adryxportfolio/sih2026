...
import { boundedSandboxCommandTimeoutMs, resolveSupervisorToken } from "@samiksha/core";
import { outgoingCorrelationHeaders } from "@samiksha/logging";
import {
...
export class DockerSandboxProvider implements SandboxProvider {
  ...
  private headers(context: AdapterContext, botId?: string) {
    return {
      ...
      "x-samiksha-space-id": context.spaceId,
      ...outgoingCorrelationHeaders(),
      ...(botId ? { "x-samiksha-bot-id": botId } : {}),
      ...(context.botId ? { "x-samiksha-screen-id": context.botId } : {}),
      ...(context.screenLeaseId ? { "x-samiksha-screen-lease-id": context.screenLeaseId } : {}),
      ...(context.cancelRunWork ? { "x-samiksha-cancel-run-work": "1" } : {}),
    };
  }
...
function dockerCwd(cwd: string | undefined) {
  if (!cwd || cwd === "." || cwd === "/" || cwd === "/home/samiksha") return "/home/samiksha";
  const relative = cwd.startsWith("/home/samiksha/")
    ? cwd.slice("/home/samiksha/".length)
    : normalizeWorkspacePath(cwd);
  return path.posix.join("/home/samiksha", relative);
}
