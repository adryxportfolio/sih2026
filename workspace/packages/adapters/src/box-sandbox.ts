...
import { boundedSandboxCommandTimeoutMs } from "@samiksha/core";
...
const BOX_WORKSPACE = "/home/user/samiksha-home";
...
export class BoxSandboxProvider implements SandboxProvider {
    ...
    const created = await this.client.create(
      {
        createBoxRequest: {
          ...
          noEnv: true,
          env: {
            SAMIKSHA_BOT_ID: request.botId,
            SAMIKSHA_SANDBOX: "computer",
          },
        },
      },
      { signal: context.signal },
  ...
  private async runCommand(
    ...
    const marker = `/tmp/samiksha-command-${randomUUID()}.completed-124`;
  ...
  private async terminateCommand(id: string, marker: string): Promise<void> {
    const pattern = marker.replace("samiksha", "[r]akazo");
    await this.rawCommand(
      id,
...
function boxCwd(cwd: string | undefined): string {
  if (
    ...
    return "samiksha-home";
  }
  const relative = cwd.startsWith(`${BOX_WORKSPACE}/`)
    ? cwd.slice(BOX_WORKSPACE.length + 1)
    : cwd.startsWith("/home/samiksha/")
      ? cwd.slice("/home/samiksha/".length)
      : cwd;
  return path.posix.join("samiksha-home", normalizeWorkspacePath(relative));
}
