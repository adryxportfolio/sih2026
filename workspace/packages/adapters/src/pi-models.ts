...
import type { ModelOAuthSignInMode, ThinkingLevel } from "@samiksha/contracts";
...
function catalogBilling(
  ...
  if (providerId === LOCAL_PROVIDER_ID) {
    return "Runs on infrastructure configured by the deployment owner. No model charges from Samiksha.";
  }
  if (providerId === OPENAI_COMPATIBLE_PROVIDER_ID) {
    return "Runs on a URL you control. Samiksha does not pay for model usage.";
  }
  if (opts.oauth && !opts.apiKey) {
    return `${name} subscription login is not in the Samiksha UI yet. Skip if this deployment already has credentials.`;
  }
  if (opts.apiKey) {
    return `Uses your ${name} API key. Samiksha does not pay for model usage.`;
  }
  return `Uses your ${name} key. Samiksha does not pay for model usage.`;
}
