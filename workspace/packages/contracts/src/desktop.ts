...
export interface SamikshaDesktopUpdate {
...
export interface SamikshaDesktopOAuthCallback {
  code: string;
  state?: string;
}

export interface SamikshaDesktop {
  ...
  oauth: {
     ...
     */
    onCallback: (listener: (callback: SamikshaDesktopOAuthCallback) => void) => () => void;
  };
}

/**
 * How the desktop app was pointed at a Samiksha server during first-run setup.
 ...
 * narrower `samikshaDesktop` bridge so a connected server can never re-point the app.
 */
export interface SamikshaSetup {
