import { ACTIVE_RUN_STATUSES, avatarIdentitySeed, organicAvatarPath } from "@samiksha/core";
...
export const BotAvatar = memo(function BotAvatar({
  ...
  const idleEyeAnimation = {
    "--samiksha-eye-animation-name": `samiksha-eyes-idle-${eyeVariant}`,
    "--samiksha-eye-animation-duration": `${idleDuration}s`,
    "--samiksha-eye-animation-easing": "cubic-bezier(0.4, 0, 0.2, 1)",
    "--samiksha-eye-animation-delay": `${idleDelay}s`,
  } as CSSProperties;
  const workingEyeAnimation = {
    "--samiksha-eye-animation-name": "samiksha-eyes-working",
    "--samiksha-eye-animation-duration": "1.4s",
    "--samiksha-eye-animation-easing": "ease-in-out",
    "--samiksha-eye-animation-delay": "0s",
  } as CSSProperties;

  return (
    <div
      className={cn(
        "samiksha-bot-avatar group relative flex items-center justify-center rounded-full select-none",
        className,
      )}
    ...
    >
      <svg
        className="samiksha-bot-avatar-ring absolute pointer-events-none"
        style={{
          inset: -4,
          width: size + 8,
          height: size + 8,
      ...
      <div
        className="samiksha-bot-avatar-visor relative flex items-center justify-center overflow-hidden transition-transform duration-200 group-hover:scale-[1.04]"
        style={{
          width: visorW,
          height: visorH,
      ...
      >
        ...
        {(["idle", "working"] as const).map((mode) => (
          <div
            key={mode}
            className={`samiksha-bot-avatar-eyes samiksha-bot-avatar-eyes-${mode} absolute inset-0 z-10 flex items-center justify-center`}
            style={{
              gap: eyeGap,
...
function OrganicAvatar({
  ...
  return (
    <svg
      ...
      aria-hidden="true"
      className={cn("samiksha-organic-avatar overflow-visible select-none", className)}
      data-working={isWorking}
    ...
    >
      {(["idle", "working"] as const).map((mode) => (
        <path
          key={mode}
          className={`samiksha-organic-avatar-body samiksha-organic-avatar-body-${mode}`}
          d={shapeA}
          fill={color}
          style={
            {
              "--samiksha-organic-path": `path("${shapeA}")`,
              filter:
                mode === "working"
      ...
      <g transform={`rotate(${(seed % 9) - 4})`}>
        {(["idle", "working"] as const).map((mode) => (
          <g
            key={mode}
            className={`samiksha-organic-avatar-eyes samiksha-organic-avatar-eyes-${mode}`}
            fill="#101014"
          >
