...
import { avatarIdentitySeed } from "@samiksha/core";
...
describe("BotAvatar", () => {
  ...
  it.each(["running", "queued", "leased", "waiting_input", "waiting_takeover"])(
    ...
    (status) => {
      ...
      expect(html).toContain("samiksha-bot-avatar-ring");
    },
  );
  ...
  it("generates an organic avatar from the bot color", () => {
    ...
    expect(html).toContain("samiksha-organic-avatar");
    expect(html).toContain('data-working="true"');
    expect(html).toMatch(/data-shape-family="\d"/);
    expect(html).toMatch(/data-eye-pattern="[0-3]"/);
    expect(html).toContain("<animate");
    expect(html).not.toContain("samiksha-bot-avatar-visor");
  });
  ...
  it("keeps the organic morph timeline stable across status updates", () => {
    ...
    expect(idle).toContain("samiksha-organic-avatar-eyes-idle");
    expect(idle).toContain("samiksha-organic-avatar-eyes-working");
    expect(idle).toContain("samiksha-organic-avatar-body-idle");
    expect(idle).toContain("samiksha-organic-avatar-body-working");
