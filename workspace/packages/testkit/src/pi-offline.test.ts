...
import type { AgentRunRequest, AgentRuntimeEvent, ConnectorTool } from "@samiksha/adapter-kit";
import { PiAgentRuntime } from "@samiksha/adapters";
...
describe("real Pi against an offline model HTTP endpoint", () => {
  it("assembles fragmented tool arguments, executes the write, and sends its result back", async () => {
    const dir = await mkdtemp(path.join(tmpdir(), "samiksha-pi-offline-"));
