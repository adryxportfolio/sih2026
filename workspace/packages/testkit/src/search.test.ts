...
import type { SearchHit, ThreadSnapshot } from "@samiksha/contracts";
...
describeSearch("workspace search", () => {
  ...
  const dataDir = mkdtempSync(path.join(tmpdir(), "samiksha-search-"));
  ...
  it("finds bots, messages, files, links, and routines within the workspace", async () => {
    const cookie = await signup(app, `search-${stamp}@samiksha.test`, "Search User");
  ...
  it("returns no hits for another workspace", async () => {
    const ownerCookie = await signup(app, `search-owner-${stamp}@samiksha.test`, "Owner");
    ...
    const intruderCookie = await signup(app, `search-intruder-${stamp}@samiksha.test`, "Intruder");
  ...
  it("finds group conversations, messages, and files", async () => {
    const cookie = await signup(app, `search-group-${stamp}@samiksha.test`, "Group Search User");
