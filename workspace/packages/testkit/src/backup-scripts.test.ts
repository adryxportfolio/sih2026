...
function fixture() {
  const root = mkdtempSync(path.join(os.tmpdir(), "samiksha-backup-"));
  ...
  for (const script of [
    ...
    if (script.endsWith("backup-prod.sh")) {
      ...
      expect(source).toContain('BACKUP_ROOT="/var/backups/samiksha"');
      source = source.replace('BACKUP_ROOT="/var/backups/samiksha"', `BACKUP_ROOT="${snapshots}"`);
    }
...
describe("development backup failures", () => {
  it.each([false, true])("archives data with a root env file present: %s", (hasEnv) => {
    ...
    expect(readFileSync(path.join(output, "samiksha.sql"), "utf8")).toContain("CREATE TABLE");
  ...
  it.each(["absent", "skipped"])("allows an empty archive when data is %s", (scenario) => {
    ...
    const result = f.run("scripts/backup.sh", ["example"], {
      SAMIKSHA_BACKUP_SKIP_HOMES: scenario === "skipped" ? "1" : "0",
    });
...
describe("production backup deployment and archive behavior", () => {
  it.each([false, true])("uses the configured deployment directory: %s", (custom) => {
    const f = fixture();
    const deployment = custom ? f.checkout : "/srv/samiksha";
    const result = f.run(
      "infra/compose/backup-prod.sh",
      [],
      custom ? { SAMIKSHA_DEPLOY_DIR: deployment } : {},
    );
    expect(result.status, result.stderr).toBe(0);
    expect(result.stdout).toContain("Verified Samiksha backup");
  ...
  it("provides optional systemd configuration for installed copies", () => {
    const service = readFileSync(
      path.join(repoRoot, "infra/systemd/samiksha-backup.service"),
      "utf8",
    );
    expect(service).toContain("EnvironmentFile=-/etc/samiksha/backup.env");
  });

  it("rejects relative deployment paths before any backup commands", () => {
    const f = fixture();
    const result = f.run("infra/compose/backup-prod.sh", [], { SAMIKSHA_DEPLOY_DIR: "relative" });
    expect(result.status).not.toBe(0);
  ...
  it.each([{ FAIL_TAR: "2" }, { FAIL_DUMP: "1" }, { FAIL_VERIFY: "1" }])(
    ...
    (failure) => {
      ...
      expect(result.status).not.toBe(0);
      expect(result.stdout).not.toContain("Verified Samiksha backup");
    },
  );
});
