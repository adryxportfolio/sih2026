...
import { createThreadEvents, createThreadMessage, loadRunHistoryMessages } from "@samiksha/db";
...
describeIntegration("run executor lifecycle", () => {
  ...
  const dataDir = mkdtempSync(path.join(tmpdir(), "samiksha-executor-lifecycle-"));
  ...
  it("records an uncertain result without replaying an interrupted external effect", async () => {
    ...
    const args = { collection: "notes", title: "Samiksha result", body: prompt };
  ...
  it("applies the same no-parallel-run rule to the targeted group member", async () => {
    const cookie = await signup(
      `executor-group-steering-${stamp}@samiksha.test`,
  ...
  async function seedRun(
    ...
    const cookie = await signup(`executor-${label}-${stamp}@samiksha.test`, `Executor ${label}`);
    const me = await rpc<{ userId: string; spaceId: string }>(cookie, "me");
