...
import { ComposioEmulator, FakeSandboxProvider } from "@samiksha/adapters";
...
describe.skipIf(!databaseAvailable)("offline Pi computer approval", () => {
  beforeAll(() => {
    ...
    vi.stubEnv("SAMIKSHA_OPENAI_COMPATIBLE_VISION_MODELS", "offline-fixture");
  });
  ...
  it.each(["allow", "deny"] as const)(
    ...
    async (answer) => {
      ...
      const dataDir = await mkdtemp(path.join(tmpdir(), "samiksha-computer-approval-"));
      ...
      try {
        ...
        const signup = await handles.app.request("/api/auth/sign-up/email", {
          ...
          body: JSON.stringify({
            email: `computer-approval-${randomUUID()}@samiksha.test`,
            password: "password12",
