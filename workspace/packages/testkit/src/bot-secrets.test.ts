...
import type { AgentRuntimeEvent } from "@samiksha/adapter-kit";
import { ScriptedAgentRuntime } from "@samiksha/adapters";
import { answerRunInput } from "@samiksha/db";
...
describeIntegration("reusable credential lifecycle", () => {
  ...
  const dataDir = mkdtempSync(path.join(tmpdir(), "samiksha-secret-lifecycle-"));
