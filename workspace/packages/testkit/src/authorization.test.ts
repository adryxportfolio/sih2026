...
import { ComposioEmulator } from "@samiksha/adapters";
import type { appContract, Space, SpaceNavigation } from "@samiksha/contracts";
import {
...
describeWithDatabase("API authorization and resource isolation", () => {
  ...
  const dataDir = mkdtempSync(path.join(tmpdir(), "samiksha-authz-"));
  ...
  it("prevents one user from reading or mutating another user's resources", async () => {
    const owner = await signup(app, `owner-authz-${stamp}@samiksha.test`, "Authorization Owner");
    const intruder = await signup(
      app,
      `intruder-authz-${stamp}@samiksha.test`,
  ...
  it("keeps approval rules private to each user in a shared Space", async () => {
    const owner = await signup(app, `approval-owner-${stamp}@samiksha.test`, "Approval Owner");
    const member = await signup(app, `approval-member-${stamp}@samiksha.test`, "Approval Member");
    const ownerActor = await rpc<Actor>(app, owner, "me");
  ...
  it("keeps space data and computers behind the selected space boundary", async () => {
    const cookie = await signup(app, `spaces-${stamp}@samiksha.test`, "Space Owner");
    ...
    const intruder = await signup(app, `spaces-intruder-${stamp}@samiksha.test`, "Intruder");
  ...
  it("enforces the space limit across concurrent creation requests", async () => {
    const cookie = await signup(app, `space-limit-${stamp}@samiksha.test`, "Space Limit");
  ...
  it("reuses provider credentials and copies their selections into a new Space", async () => {
    const cookie = await signup(app, `space-provider-copy-${stamp}@samiksha.test`, "Provider Copy");
    const actor = await rpc<Actor>(app, cookie, "me");
  ...
  it("shares model credentials while keeping defaults private to each space", async () => {
    const cookie = await signup(app, `model-defaults-${stamp}@samiksha.test`, "Model Defaults");
    const actor = await rpc<Actor>(app, cookie, "me");
  ...
  it("deletes only empty, non-default spaces", async () => {
    const cookie = await signup(app, `space-delete-${stamp}@samiksha.test`, "Space Delete");
    const actor = await rpc<Actor>(app, cookie, "me");
    ...
    const intruder = await signup(app, `space-delete-intruder-${stamp}@samiksha.test`, "Intruder");
    ...
    const memberCookie = await signup(
      app,
      `space-delete-member-${stamp}@samiksha.test`,
      "Space Member",
    );
  ...
  it("blocks bot creation after empty space deletion is claimed", async () => {
    const cookie = await signup(app, `space-race-${stamp}@samiksha.test`, "Space Race");
    const actor = await rpc<Actor>(app, cookie, "me");
  ...
  it("keeps a space claimed after ambiguous sandbox teardown failure", async () => {
    const cookie = await signup(app, `space-teardown-${stamp}@samiksha.test`, "Teardown");
    const actor = await rpc<Actor>(app, cookie, "me");
  ...
  it("times out a hung sandbox teardown without unblocking the space", async () => {
    const cookie = await signup(app, `space-deadline-${stamp}@samiksha.test`, "Deadline");
    const actor = await rpc<Actor>(app, cookie, "me");
  ...
  it("validates custom thinking against the saved connection capability", async () => {
    const cookie = await signup(app, `custom-thinking-${stamp}@samiksha.test`, "Custom Thinking");
  ...
  it("validates per-bot model overrides against connected providers and catalog", async () => {
    const cookie = await signup(app, `bot-model-${stamp}@samiksha.test`, "Bot Model");
    const bot = await rpc<
      Bot & {
  ...
  it("chooses the newest duplicate provider credential when selecting a default", async () => {
    const cookie = await signup(app, `model-duplicates-${stamp}@samiksha.test`, "Model Duplicates");
    const actor = await rpc<Actor>(app, cookie, "me");
  ...
  it("restricts deployment settings to the deployment owner", async () => {
    const owner = await signup(app, `deployment-owner-${stamp}@samiksha.test`, "Deployment Owner");
    const other = await signup(app, `deployment-other-${stamp}@samiksha.test`, "Deployment Other");
    const ownerActor = await rpc<Actor>(app, owner, "me");
    ...
    try {
      ...
      const closedSignup = await app.request("/api/auth/sign-up/email", {
        ...
        body: JSON.stringify({
          email: `closed-${stamp}@samiksha.test`,
      ...
      const disallowedSignup = await app.request("/api/auth/sign-up/email", {
        ...
        headers: { "content-type": "application/json" },
        body: JSON.stringify({
          email: `not-approved-${stamp}@samiksha.test`,
          password: "password123",
...
async function raw(
  ...
  return app.request(`/rpc/${procedure}`, {
    ...
    headers: {
      ...
      ...(cookie ? { cookie } : {}),
      ...(spaceId ? { "x-samiksha-space-id": spaceId } : {}),
