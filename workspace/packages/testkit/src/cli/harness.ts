...
async function main() {
  ...
  try {
    ...
    execSync("pnpm --filter @samiksha/db generate", { stdio: "inherit", env: process.env });
    execSync("pnpm --filter @samiksha/db exec prisma migrate deploy", {
      stdio: "inherit",
      env: process.env,
    ...
    const [
      ...
      { createApp },
    ] = await Promise.all([import("@samiksha/adapters"), import("../../../../apps/api/src/app.ts")]);
