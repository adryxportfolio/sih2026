...
import { ModelConnectInputSchema } from "@samiksha/contracts";
...
async function main() {
  ...
  const dataDir = mkdtempSync(path.join(tmpdir(), "samiksha-evals-"));
  ...
  try {
    ...
    execFileSync("pnpm", ["--filter", "@samiksha/db", "generate"], {
      stdio: "pipe",
      timeout: 120_000,
    });
