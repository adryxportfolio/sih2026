...
async function main() {
  ...
  execFileSync("pnpm", ["--filter", "@samiksha/db", "exec", "prisma", "migrate", "deploy"], {
    ...
    for (const flow of ["screenshots", "notification-demo"]) {
      await runProcess(
        ...
        [
          ...
          "-e",
          `SAMIKSHA_SCREENSHOT_EMAIL=${EMAIL}`,
          "-e",
          `SAMIKSHA_SCREENSHOT_PASSWORD=${PASSWORD}`,
          "-e",
          `SAMIKSHA_SCREENSHOT_BOT_ID=${fixture.botId}`,
          "-e",
          `SAMIKSHA_SCREENSHOT_GROUP_ID=${fixture.groupId}`,
          "-e",
          `SAMIKSHA_SCREENSHOT_ROUTINE_ID=${fixture.routineId}`,
          "-e",
          `SAMIKSHA_NOTIFICATION_VIDEO=${path.join(REPORT_DIR, "notification-demo")}`,
          "-e",
          `SAMIKSHA_EXPAND_NOTIFICATIONS_URL=${EXPAND_NOTIFICATIONS_URL}`,
