...
import { abortableDelay } from "@samiksha/core";
import { loadRootEnv } from "@samiksha/core/node/load-root-env";
import { createThreadMessage, type PrismaClient } from "@samiksha/db";
...
const temporaryRoot = await mkdtemp(path.join(os.tmpdir(), "samiksha-desktop-performance-"));
...
function performanceEnvironment(databaseUrl: string): NodeJS.ProcessEnv {
  return {
    ...
    E2B_API_KEY: "",
    DAYTONA_API_KEY: "",
    BETTER_AUTH_SECRET: "samiksha-benchmark-auth-secret-over-32-characters",
    ENCRYPTION_KEY: "samiksha-benchmark-encryption-key-over-32-characters",
    SANDBOX_SUPERVISOR_TOKEN: "samiksha-benchmark-supervisor-token-over-32-characters",
    SCREEN_PROXY_SECRET: "samiksha-benchmark-screen-proxy-secret-over-32-characters",
    ...
    SAMIKSHA_HOST: "127.0.0.1",
    SAMIKSHA_WEB_URL: webOrigin,
    SAMIKSHA_DISABLE_BUNDLED_RENDERER: remoteRenderer ? "1" : "0",
    SAMIKSHA_DISABLE_WARM_WINDOW: disableWarmWindow ? "1" : "0",
    SAMIKSHA_PERFORMANCE_ASSET_DELAY_MS: String(assetDelayMs),
...
function buildProductionArtifacts(env: NodeJS.ProcessEnv) {
  run("pnpm", ["--filter", "@samiksha/desktop", "pack:dir"], env);
}

function migrateDatabase(env: NodeJS.ProcessEnv) {
  run("pnpm", ["--filter", "@samiksha/db", "generate"], env);
  run("pnpm", ["--filter", "@samiksha/db", "exec", "prisma", "migrate", "deploy"], env);
}
...
async function packagedExecutable() {
  ...
  const candidates = process.platform === "darwin" ? await findNamed(out, "Samiksha.app") : [];
  if (process.platform === "darwin" && candidates[0]) {
    return path.join(candidates[0], "Contents/MacOS/Samiksha");
  }
...
async function prepareAuthenticatedProfile(benchmark: BenchmarkContext, profile: string) {
  ...
  try {
    ...
    await page.getByPlaceholder("Your email address").fill(`benchmark-${stamp}@samiksha.test`);
...
async function launchDesktop(
  ...
  const app = await electron.launch({
    ...
    env: {
      ...benchmark.env,
      SAMIKSHA_PERFORMANCE_USER_DATA: profile,
      SAMIKSHA_PERFORMANCE_CLEAR_CACHE: clearCache ? "1" : "0",
    },
  });
...
async function measureInteractions(app: ElectronApplication, page: Page) {
  ...
  await page.evaluate(() => {
    ...
    (window as typeof window & { __samikshaKeyPaintSamples?: number[] }).__samikshaKeyPaintSamples =
      samples;
...
async function measureBundles() {
  ...
  const applications = await findNamed(path.join(desktopRoot, "out"), "Samiksha.app");
...
function renderMarkdown(report: PerformanceReport) {
  ...
  return `# Samiksha desktop performance — ${report.label}
