...
import type { RunsListOutput, ThreadSnapshot } from "@samiksha/contracts";
import { isTerminal } from "@samiksha/core";
...
const project = `samiksha-topology-${process.pid}-${Date.now().toString(36)}`;
...
async function main() {
  ...
  const env = {
    ...
    TOPOLOGY_APP_IMAGE: process.env.TOPOLOGY_APP_IMAGE ?? "samiksha/app:topology",
    TOPOLOGY_COMPUTER_IMAGE: process.env.TOPOLOGY_COMPUTER_IMAGE ?? "samiksha/computer:topology",
  };
...
async function signup(baseUrl: string) {
  const response = await fetch(`${baseUrl}/api/auth/sign-up/email`, {
    ...
    body: JSON.stringify({
      email: `topology-${Date.now()}@samiksha.test`,
      password: "password12",
      name: "Topology",
    }),
  });
...
async function waitForWorker(
  ...
  while (Date.now() - started < timeoutMs) {
    ...
    // {"timestamp":...,"level":"info","message":"worker ready","service.name":"samiksha-worker"},
    // not the old plain "samiksha worker ready" console.log line -- match the JSON field directly
...
function assertComputerNetworkIsolation(
  ...
  const managed = docker([
    ...
    computerId,
    "--format",
    '{{index .Config.Labels "samiksha.managed"}}',
  ]).trim();
