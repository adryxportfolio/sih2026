...
async function main() {
  ...
  if (unknown.length)
    throw new Error(
      "Usage: computer-replay.ts [--image=samiksha/computer:local] [--subnet=unused-private-CIDR] [--live --record=new-fixture.json]",
    );
  const image = imageArg?.slice("--image=".length) || "samiksha/computer:local";
  ...
  const directory = await mkdtemp(path.join(tmpdir(), "samiksha-computer-replay-"));
