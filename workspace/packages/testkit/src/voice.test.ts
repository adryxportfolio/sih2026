...
import { SCRIPTED_MPEG, SCRIPTED_TRANSCRIPT, SCRIPTED_VOICE_ID } from "@samiksha/adapters";
...
describeVoice("voice credentials and speech HTTP", () => {
  ...
  const dataDir = mkdtempSync(path.join(tmpdir(), "samiksha-voice-"));
  ...
  it("connects a scripted key, speaks, and transcribes without leaking the secret", async () => {
    const cookie = await signup(app, `voice-${stamp}@samiksha.test`, "Voice User");
    const other = await signup(app, `voice-other-${stamp}@samiksha.test`, "Other Voice");
