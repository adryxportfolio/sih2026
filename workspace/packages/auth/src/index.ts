import type { TransactionalEmail, TransactionalEmailProvider } from "@samiksha/adapter-kit";
import { emailAllowed, isMessagingEmail, parseAllowlist, signupPolicyFromEnv } from "@samiksha/core";
import { bootstrapUserSpace, type PrismaClient } from "@samiksha/db";
...
import {
  ...
  type SupabaseIdentityConfig,
  verifySupabaseCredentials,
} from "./supabase-identity.js";
...
export interface AuthEnv {
  ...
  /**
   * When set, Supabase is the sole authority for officer credentials and the
   * local credential store becomes a cache of its decisions.
   */
  supabase?: SupabaseIdentityConfig;
}
      ...
      before: createAuthMiddleware(async (ctx) => {
        ...
        let policy =
          ctx.path === "/sign-up/email" || ctx.path === "/sign-in/email"
            ...
            : undefined;
        if (ctx.path === "/sign-in/email" && env.supabase) {
          // Prove the credentials against Supabase before Better Auth consults
          // its own store, then reconcile the local row so its check agrees.
          const email = String(ctx.body?.email ?? "").trim();
          const password = String(ctx.body?.password ?? "");
          if (!email || !password) {
            throw new APIError("UNAUTHORIZED", { message: "Invalid email or password" });
          }
          let identity: Awaited<ReturnType<typeof verifySupabaseCredentials>>;
          try {
            identity = await verifySupabaseCredentials(env.supabase, email, password);
          } catch {
            throw new APIError("INTERNAL_SERVER_ERROR", {
              message: "Could not reach the Samiksha identity service. Try again.",
            });
          }
          if (!identity) {
            throw new APIError("UNAUTHORIZED", { message: "Invalid email or password" });
          }
          await reconcileLocalIdentity(prisma, identity, password);
        }
      ...
      session: {
        create: {
          before: async (session, ctx) => {
            ...
            if (!membership) {
              // With Supabase as the identity authority, reaching session
              // creation already means an administrator provisioned this
              // officer and their credentials just verified. Registration being
              // closed is what stops self-signup; it must not strand a
              // legitimate officer without the workspace they were granted.
              if (!env.supabase && (!policy.enabled || !emailAllowed(user.email, policy.allowlist))) {
...
export function verificationEmail(email: string, url: string): TransactionalEmail {
  return {
    to: email,
    subject: "Verify your Samiksha email",
    text: `Verify your email, then return to Samiksha to sign in:\n\n${url}\n\nThis link expires in one hour. If you did not register, ignore this email.`,
    html: `<p><a href="${escapeHtml(url)}">Verify email</a>, then return to Samiksha to sign in.</p><p>This link expires in one hour. If you did not register, ignore this email.</p>`,
  };
}
...
export function passwordResetEmail(
  ...
  return {
    to: user.email,
    subject: "Reset your Samiksha password",
    text: [
      `Hi ${name},`,
      "",
      "Reset your Samiksha password using this link:",
      resetUrl,
      "",
      "This link expires in one hour. If you did not request this, you can ignore this email.",
    ].join("\n"),
    html: `<p>Hi ${safeName},</p><p>Reset your Samiksha password:</p><p><a href="${safeUrl}">Reset password</a></p><p>This link expires in one hour. If you did not request this, you can ignore this email.</p>`,
  };
}
