...
import { provisionDefaultAgents } from "./default-agents.js";
...
export async function bootstrapUserSpace(
  ...
  // The officer should land in a workspace that already works. There is no
  // onboarding step to create an assistant in, so the starter team is part of
  // being granted an account.
  const email =
    user.email ??
    (await prisma.user.findUnique({ where: { id: user.id }, select: { email: true } }))?.email;
  if (email) {
    await provisionDefaultAgents(prisma, { userId: user.id, spaceId: orgId, email });
  }
