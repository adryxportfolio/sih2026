...
import { readBoundedJsonResponse, signupRequiresEmailVerification } from "@samiksha/core";
import { Button, Input, Label } from "@samiksha/ui-web";
...
export function AuthPage({ mode }: { mode: AuthMode }) {
  ...
  const title = sent ? (
    ...
    <Trans>Sign in to Samiksha</Trans>
  ) : mode === "up" ? (
    <Trans>Create your Samiksha</Trans>
  ) : (
      ...
      {sent ? (
        ...
        <>
          ...
          <p className="mt-8 text-muted-foreground">
            {mode === "in" ? (
              // Officers are issued accounts by an administrator against a
              // verified employee ID, so there is nothing to sign up for.
              <Trans>
                Use the credentials your administrator issued you. Same sign-in as the Samiksha
                mobile app.
              </Trans>
            ) : (
              <Link to="/sign-in" className="font-medium text-foreground">
