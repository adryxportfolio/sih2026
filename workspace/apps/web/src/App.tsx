...
import { LOCAL_SETTINGS_PAGE } from "@samiksha/contracts";
import { Button, Skeleton } from "@samiksha/ui-web";
...
function SessionApp() {
  ...
  if (gate === "loading") {
    return window.location.pathname.startsWith("/app") ? (
      ...
      <div
        ...
        data-samiksha-app-state="session-pending"
      >
        <Trans>Loading…</Trans>
      </div>
    );
  ...
  return (
    <div className="h-full" data-samiksha-app-state="ready">
      <Suspense fallback={<div className="h-full bg-background" />}>
        <Routes>
          ...
          />
          {/* No self-registration: an administrator provisions every officer. */}
          ...
          {/* The starter agent team is provisioned at first sign-in, so there
              is no setup step between signing in and working. */}
          <Route path="/onboarding" element={<Navigate to={user ? "/app" : "/sign-in"} replace />} />
          <Route
