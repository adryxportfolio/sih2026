...
import type { AppContract } from "@samiksha/contracts";
import { LOCAL_SETTINGS_PAGE, LOCAL_SETTINGS_RPC } from "@samiksha/contracts";
import { desktopBridge } from "./desktop";

const SPACE_STORAGE_KEY = "samiksha:space-id";
...
/** Adds `x-samiksha-space-id` when a space is selected. */
export function withSpaceHeaders(
  init?: HeadersInit,
  spaceId: string | null = selectedSpaceId(),
): Headers {
  const headers = new Headers(init);
  if (spaceId) headers.set("x-samiksha-space-id", spaceId);
  else headers.delete("x-samiksha-space-id");
  return headers;
}
