...
function stubSelectedSpace(id: string) {
  const store = new Map<string, string>([["samiksha:space-id", id]]);
  const localStorage = {
  ...
  return (next: string) => store.set("samiksha:space-id", next);
}
...
describe("Speaker", () => {
  ...
  it("keeps speak requests in the space where playback started", async () => {
    ...
    expect(headers.get("x-samiksha-space-id")).toBe("space-support");
