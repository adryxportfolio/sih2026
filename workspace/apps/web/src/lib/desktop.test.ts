...
import {
  ...
  type SamikshaDesktop,
  type SamikshaDesktopOAuthCallback,
  windowChromeKind,
} from "./desktop.js";

function desktop(platform: string): SamikshaDesktop {
  const updateState = {
...
describe("attempt correlation", () => {
  ...
  function bridgeEmitting() {
    let emit: (callback: SamikshaDesktopOAuthCallback) => void = () => undefined;
    const unsubscribe = vi.fn();
    vi.stubGlobal("window", {
      samikshaDesktop: {
        ...desktop("linux"),
        oauth: {
          onCallback: (listener: (callback: SamikshaDesktopOAuthCallback) => void) => {
            emit = listener;
            return unsubscribe;
          },
        },
    ...
    return { emit: (c: SamikshaDesktopOAuthCallback) => emit(c), unsubscribe };
  }
