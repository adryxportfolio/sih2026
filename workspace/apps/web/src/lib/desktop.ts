import type { SamikshaDesktop, SamikshaDesktopOAuthCallback } from "@samiksha/contracts";

export type { SamikshaDesktop, SamikshaDesktopOAuthCallback } from "@samiksha/contracts";

declare global {
  interface Window {
    samikshaDesktop?: SamikshaDesktop;
  }
}

export function desktopBridge(): SamikshaDesktop | undefined {
  return typeof window === "undefined" ? undefined : window.samikshaDesktop;
}

/** The compact `code#state` form the manual paste flow already accepts. */
export function desktopOAuthCode(callback: SamikshaDesktopOAuthCallback) {
...
export function windowChromeKind(desktop?: SamikshaDesktop): "spacer" | "darwin" | "controls" {
