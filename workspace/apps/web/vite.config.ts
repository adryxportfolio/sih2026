...
import type { DesktopStackProbeResponse } from "@samiksha/contracts";
import {
  safeScreenProxyResponseHeaders,
  stripSensitiveHandshakeHeaders,
} from "@samiksha/core/node/screen-proxy-response";
...
export default defineConfig(({ mode }) => {
  ...
  const previewHost = process.env.SAMIKSHA_HOST ?? rootEnv.SAMIKSHA_HOST ?? "localhost";
  ...
  const performanceAssetDelayMs = Number(process.env.SAMIKSHA_PERFORMANCE_ASSET_DELAY_MS ?? 0);
  const desktopStackToken =
    process.env.SAMIKSHA_DESKTOP_STACK_TOKEN ?? rootEnv.SAMIKSHA_DESKTOP_STACK_TOKEN ?? "";
  const imageTag = process.env.SAMIKSHA_IMAGE_TAG ?? rootEnv.SAMIKSHA_IMAGE_TAG ?? "edge";
  return {
    plugins: [
      react(),
      ...
      tailwindcss(),
      {
        name: "samiksha-desktop-stack-probe",
      ...
      {
        name: "samiksha-performance-asset-delay",
        configurePreviewServer(server) {
