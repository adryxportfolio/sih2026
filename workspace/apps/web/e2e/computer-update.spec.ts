...
import type { ComputerUpdate } from "@samiksha/contracts";
...
test("computer maintenance shows durable background progress and failure recovery", async ({
  ...
  await page.addInitScript(() => localStorage.setItem("samiksha.uiAppearance", "dark"));
  await signup(page, `computer-update-${Date.now()}@samiksha.test`, "password12", "Computer Update");
