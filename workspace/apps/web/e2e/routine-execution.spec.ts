...
import type { Routine } from "@samiksha/contracts";
...
test("Slack message trigger uses the mounted messaging provider and persists", async ({
  ...
  await signup(page, `routine-slack-${stamp}@samiksha.test`, "password12", "Slack Routine");
...
test("GitHub event trigger exposes signed delivery settings and persists", async ({
  ...
  const stamp = Date.now();
  await signup(page, `routine-github-${stamp}@samiksha.test`, "password12", "GitHub Routine");
  await completeOnboarding(page);
  const botId = activeBotId(page);
...
test("Korean webhook routine keeps technical field labels in English", async ({
  ...
  await signup(page, `routine-ko-${stamp}@samiksha.test`, "password12", userName);
  await completeOnboarding(page);
...
test("routine test-run completes and survives reload", async ({ page }, testInfo) => {
  const stamp = Date.now();
  await signup(page, `routine-${stamp}@samiksha.test`, "password12", "Routine");
  await completeOnboarding(page);
