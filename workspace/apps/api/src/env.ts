...
import { supabaseIdentityFromEnv, type SupabaseIdentityConfig } from "@samiksha/auth";
import {
  resolveAuthSecret,
...
export { resolveCloudAgentProvider, resolveSandboxProvider } from "@samiksha/adapters";
...
export interface AppEnv {
  ...
  /** Supabase project backing officer identity; undefined disables the bridge. */
  supabase: SupabaseIdentityConfig | undefined;
...
export function loadEnv(source: NodeJS.ProcessEnv = process.env): AppEnv {
  ...
  const updaterUrl = optional(source.SAMIKSHA_UPDATER_URL);
  const updaterToken = optional(source.SAMIKSHA_UPDATER_TOKEN);
  return {
    nodeEnv: source.NODE_ENV ?? "",
    databaseUrl: required(source, "DATABASE_URL"),
    realtimeDatabaseUrl: source.REALTIME_DATABASE_URL ?? required(source, "DATABASE_URL"),
    desktopStackToken: optional(source.SAMIKSHA_DESKTOP_STACK_TOKEN),
    authSecret,
    ...
    supabase: supabaseIdentityFromEnv(source),
    ...
    teamChatBotId: optional(source.TEAM_CHAT_BOT_ID) ?? optional(source.SLACK_SAMIKSHA_BOT_ID),
    ...
    gitSha: optional(source.GIT_SHA) ?? optional(source.SAMIKSHA_GIT_SHA),
    updaterUrl,
    updaterToken,
    imageTag: optional(source.SAMIKSHA_IMAGE_TAG),
  };
}
