...
import { ServerUpdateRunSchema } from "@samiksha/contracts";
import {
...
export interface UpdaterProxyConfig {
  ...
  /** Current `SAMIKSHA_IMAGE_TAG` when known; selects compose pull vs rebuild commands. */
...
export async function readServerUpdateStatus(
  ...
  const imageTagHint = config.imageTag?.trim() || process.env.SAMIKSHA_IMAGE_TAG?.trim() || null;
