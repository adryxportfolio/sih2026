import type { SandboxProvider } from "@samiksha/adapter-kit";
...
describe("stopThreadRuns", () => {
  it("snapshots every lease when group members share a team computer", async () => {
    ...
    expect(execute).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        argv: expect.arrayContaining(["samiksha-cancel-run-work", "computer-db-team", "run-a"]),
      }),
    ...
    expect(execute).toHaveBeenCalledWith(
      expect.objectContaining({ providerRef: "computer-team" }),
      expect.objectContaining({
        argv: expect.arrayContaining(["samiksha-cancel-run-work", "computer-db-team", "run-b"]),
      }),
  ...
  it("does not tear down a stale legacy execution run when the lease owns a cancelled run", async () => {
    ...
    expect(execute).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        argv: expect.arrayContaining(["samiksha-cancel-run-work", "computer-db-a", "run-a"]),
      }),
      expect.objectContaining({ cancelRunWork: true, runId: "run-a", botId: "bot-a" }),
    );
    expect(execute).not.toHaveBeenCalledWith(
      expect.anything(),
      expect.objectContaining({
        argv: expect.arrayContaining(["samiksha-cancel-run-work", "computer-db-a", "run-b"]),
      }),
      expect.anything(),
    );
