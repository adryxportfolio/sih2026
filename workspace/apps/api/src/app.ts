...
import { blockedAuthPaths, createAuth } from "@samiksha/auth";
import { signupPolicyFromEnv } from "@samiksha/core";
import {
  createDb,
  createThreadEvents,
  type PrismaClient,
...
export async function createApp(
  ...
  app.use("/rpc/*", async (c, next) => {
    ...
    const requestedSpaceId = c.req.header("x-samiksha-space-id");
...
function isTrustedOrigin(origin: string, env: AppEnv) {
  ...
  if (origin.startsWith("samiksha://") || origin.startsWith("exp://")) return true;
  try {
