...
import { runContinueJob } from "@samiksha/adapter-kit";
import { AutomatedSenderPoliciesSchema, type MessageBlock } from "@samiksha/contracts";
import { BOT_MESSAGE_MAX_HOPS } from "@samiksha/core";
...
export class TeamChatBridge {
  ...
  async receive(
    ...
    if (
      ...
      throw new Error("Team chat conversation belongs to a different Samiksha target");
    }
    const now = new Date();
  ...
  private async ensureTranscriptMessage(
    ...
    if (!conversation.thread) throw new Error("Team chat conversation has no Samiksha thread");
  ...
  private async deliverDelegatedReplies(target: TargetBot): Promise<void> {
    ...
    for (const run of runs) {
      ...
      const content =
        run.status === "failed"
          ? `${target.name} could not complete the delegated request. Open Samiksha for details.`
  ...
  private async queue(message: {
    ...
    if (!thread) throw new Error("Team chat conversation has no Samiksha thread");
  ...
  private async deliverFailure(message: {
    ...
    await this.sendOnce(message.id, {
      conversationId: message.externalConversation.conversationId,
      replyThreadId: message.replyThreadId,
      content: `${this.target?.name ?? "The agent"} could not complete that request. Open Samiksha for details.`,
