...
import type { DesktopLocalStackState } from "@samiksha/contracts";
import {
...
export const STACK_PROJECT_NAME = "samiksha-desktop";
...
const LAUNCH_SUPPLIED = ["SAMIKSHA_IMAGE_TAG", "SAMIKSHA_COMPUTER_IMAGE_TAG"];
  ...
  switch (kind) {
    ...
    case "other":
      return phase === "pulling"
        ? "Downloading Samiksha images failed. Check the output below, then retry."
        : "Samiksha services did not start. Check the output below, then retry.";
  }
}
  ...
  private docker(binary: string, args: string[], timeoutMs: number, signal?: AbortSignal) {
    return this.deps.run(binary, args, {
      ...
      env: dockerSpawnEnv(this.deps.platform, this.deps.env, binary, {
        SAMIKSHA_IMAGE_TAG: this.deps.imageTag,
        SAMIKSHA_COMPUTER_IMAGE_TAG: this.deps.imageTag,
        ...(this.currentStackToken === null
          ? {}
          : { SAMIKSHA_DESKTOP_STACK_TOKEN: this.currentStackToken }),
        SAMIKSHA_WEB_PORT: new URL(this.currentWebUrl).port || "80",
