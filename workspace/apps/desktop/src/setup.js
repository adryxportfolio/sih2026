(() => {
  const bridge = window.samikshaSetup;
  ...
  const PHASE_LABELS = {
    ...
    pulling: "Downloading Samiksha…",
    starting: "Starting Samiksha…",
    "waiting-healthy": "Almost ready…",
    ready: "Samiksha is ready.",
  };
  ...
  async function check() {
    ...
    try {
      ...
      if (result.ok) {
        ...
        setStatus(`Samiksha answered at ${result.url}.`, "ok");
      } else {
  ...
  async function init() {
    ...
    try {
      ...
      setStatus("Setup could not start. Quit Samiksha and try again.", "error");
      setBusy(true);
    }
  }
