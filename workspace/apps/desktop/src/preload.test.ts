...
import type { SamikshaDesktop, SamikshaSetup } from "@samiksha/contracts";
...
describe("desktop preload bridge", () => {
  it("exposes the scoped desktop bridges", async () => {
    ...
    const [globalName, bridge] = exposeInMainWorld.mock.calls[0] as [string, SamikshaDesktop];
    expect(globalName).toBe("samikshaDesktop");
  ...
  it("forwards captured codes without leaking the IPC event to the renderer", () => {
    ...
    const [, bridge] = exposeInMainWorld.mock.calls[0] as [string, SamikshaDesktop];
...
describe("setup preload bridge", () => {
  it("exposes only the first-run setup operations", async () => {
    ...
    expect(exposeInMainWorld).toHaveBeenCalledTimes(1);
    const [globalName, bridge] = exposeInMainWorld.mock.calls[0] as [string, SamikshaSetup];
    expect(globalName).toBe("samikshaSetup");
    expect(bridge.platform).toBe("linux");
    expect(Object.keys(bridge).sort()).toEqual([
      "openLink",
      "platform",
