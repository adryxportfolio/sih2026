...
import type { DesktopSetup, DesktopStackProbeResponse } from "@samiksha/contracts";
...
/**
 * Accepts what a person would actually type ("localhost:5173", "samiksha.example.com")
 * and returns a canonical http(s) origin, or null when the input can never
 * securely address a Samiksha server.
 */
export function normalizeServerUrl(input: string): string | null {
  ...
  // Samiksha serves its renderer, RPC, and auth routes from one origin. Keeping a
 ...
 * `SAMIKSHA_WEB_URL` still wins over saved configuration so test and performance
...
/** Each Samiksha origin gets its own persistent cookie and storage partition. */
export function sessionPartitionForServerUrl(targetUrl: string): string | null {
  try {
    ...
    return `persist:samiksha-${digest}`;
  } catch {
    return null;
  }
}
...
export function isSamikshaHealth(value: unknown): boolean {
 ...
 * metadata endpoints. Cleartext HTTP to them is never a legitimate Samiksha
