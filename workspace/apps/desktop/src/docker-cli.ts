...
import type { DesktopSetupLink } from "@samiksha/contracts";
...
/**
 * `SAMIKSHA_DOCKER_BINARY` is a test hook and the only candidate when set, so a
...
export function resolveDockerBinary(
  ...
  const override = env.SAMIKSHA_DOCKER_BINARY?.trim();
