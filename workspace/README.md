...
[![GitHub stars](https://img.shields.io/github/stars/elie222/samiksha?labelColor=black&style=for-the-badge&color=2563EB)](https://github.com/elie222/samiksha/stargazers)
...
![Samiksha — AI teammates you actually own](./docs/readme-hero.png)

Samiksha is an open-source platform for running persistent AI teammates. It is available on the web,
...
Samiksha is in beta. Learn more at [samiksha.com](https://samiksha.com).
...
The installer downloads the Compose files, creates `.env` with random secrets, and starts Samiksha.
...
On restricted networks, override the installer download base (`SAMIKSHA_DOWNLOAD_BASE`), skip
existing Compose files (`--local` / `SAMIKSHA_DOWNLOAD_SKIP_EXISTING`), or mirror the bootstrap
script URL — see
...
# edit .env: SANDBOX_PROVIDER=box (or e2b / daytona) with its API key, SAMIKSHA_HOST=your.domain
bash install-images.sh
```
...
```bash
git clone https://github.com/elie222/samiksha.git
cd samiksha
cp .env.example .env
```
...
The Electron and Expo apps are clients of the same Samiksha API used by the web app.

With the development stack running, launch Electron with:

```bash
pnpm --filter @samiksha/desktop dev
```

On first run the desktop app asks whether to run Samiksha on this computer or connect to an existing
...
app verifies Samiksha's health endpoint before saving, and later launches go straight to that
instance. The stack keeps running after the app quits; **Stop Local Stack** in the application
menu turns it off.

Use **Change Samiksha Server…** in the application menu to reconnect. Closing that window without
saving returns to the previous instance. For development automation, set `SAMIKSHA_WEB_URL` to point
the shell somewhere else without changing the saved instance, or `SAMIKSHA_FORCE_SETUP=1` to run
setup again.
...
Samiksha is licensed under the [Apache License 2.0](./LICENSE).

Questions and ideas are welcome in the [Samiksha Discord community](https://discord.gg/RWwKa2Sn7h).
