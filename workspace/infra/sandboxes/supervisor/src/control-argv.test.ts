...
import { browserProfilePathForScreen } from "@samiksha/core/node/desktop-runtime";
  ...
  (browserProfile) => {
    it.each([
      ...
      if (
        ...
        expect(step.argv[2]).toBe(`SAMIKSHA_BROWSER_PROFILE=${browserProfile}`);
      }
...
describe("controller argv restrictions", () => {
  it.each([
    ["env", `DISPLAY=${display}`, `SAMIKSHA_BROWSER_PROFILE=${profile}`],
    ["env", `DISPLAY=${display}`, "SAMIKSHA_BROWSER_PROFILE=", "samiksha-browser"],
    ["env", `DISPLAY=${display}`, `SAMIKSHA_BROWSER_PROFILE=${profile}a`, "samiksha-browser"],
    [
      "env",
      `DISPLAY=${display}`,
      `SAMIKSHA_BROWSER_PROFILE=${profile.toUpperCase()}`,
      "samiksha-browser",
    ],
    ["env", `DISPLAY=${display}`, `SAMIKSHA_BROWSER_PROFILE=${profile}`, "xterm"],
    [
      "env",
      `DISPLAY=${display}`,
      `SAMIKSHA_BROWSER_PROFILE=${profile}`,
      "/usr/bin/xdg-open",
      "https://example.com",
    ],
    ["env", `DISPLAY=${display}`, `SAMIKSHA_BROWSER_PROFILE=${profile}`, "xdg-open"],
    [
      "env",
      `DISPLAY=${display}`,
      `SAMIKSHA_BROWSER_PROFILE=${profile}`,
    ...
    ["env", "DISPLAY=:8", `SAMIKSHA_BROWSER_PROFILE=${profile}`, "samiksha-browser"],
    ["env", "DISPLAY=:8", "xdg-open", "https://example.com"],
    ["env", `DISPLAY=${display}`, "LD_PRELOAD=/tmp/unsafe", "xdg-open", "https://example.com"],
    ["env", `DISPLAY=${display}`, "SAMIKSHA_BROWSER_PROFILE=/tmp/unsafe", "samiksha-browser"],
    ["env", `DISPLAY=${display}`, `SAMIKSHA_BROWSER_PROFILE=${profile}/../other`, "samiksha-browser"],
    ["env", `DISPLAY=${display}`, `SAMIKSHA_BROWSER_PROFILE=${profile}`, "sh", "-c", "true"],
    ["env", `DISPLAY=${display}`, `SAMIKSHA_BROWSER_PROFILE=${profile}`, "xdotool", "key", "Return"],
    [
      "env",
      `DISPLAY=${display}`,
      `SAMIKSHA_BROWSER_PROFILE=${profile}`,
