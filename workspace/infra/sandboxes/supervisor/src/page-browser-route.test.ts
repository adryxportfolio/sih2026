...
beforeEach(() => {
  ...
  mock.inspect.mockResolvedValue({
    Config: {
      Labels: { "samiksha.managed": "true", "samiksha.botId": "home", "samiksha.spaceId": "space" },
    },
  });
  mock.exec.mockImplementation(async (options: { Cmd: string[] }) => ({
    start: async () =>
      Readable.from([
        Buffer.from(
          options.Cmd.includes("/usr/local/bin/samiksha-page-browser")
            ? JSON.stringify({
                ok: true,
...
async function snapshot(id: string, screen: string, lease: string, home = "home") {
  return supervisorApp.request(`/computers/${id}/browser`, {
    ...
    headers: {
      ...
      "x-samiksha-bot-id": home,
      "x-samiksha-space-id": "space",
      "x-samiksha-screen-id": screen,
      "x-samiksha-screen-lease-id": lease,
    },
...
it("closes helper stdin when the request is cancelled", async () => {
  ...
  mock.exec.mockImplementation(async (options: { Cmd: string[]; AttachStdin?: boolean }) => {
    if (!options.Cmd.includes("/usr/local/bin/samiksha-page-browser")) return defaultExec(options);
