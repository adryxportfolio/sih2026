...
function fixture() {
  const root = mkdtempSync(path.join(tmpdir(), "samiksha-profile-test-"));
  fixtures.push(root);
  ...
  mkdirSync(runtime);
  const profile = (bot: string) => browserProfilePathForScreen(bot).replace("/home/samiksha", home);
  const run = (script: string) => {
    ...
    return spawnSync(
      "bash",
      ["-eu", "-c", portable.replaceAll("/tmp/samiksha", runtime).replaceAll("/home/samiksha", home)],
      {
        encoding: "utf8",
        timeout: 10_000,
      },
