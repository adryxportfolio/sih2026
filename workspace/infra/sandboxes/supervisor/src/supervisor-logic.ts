...
} from "@samiksha/core/node/desktop-runtime";
...
import { canReleaseScreenLease, canTakeScreenLease } from "@samiksha/core";
import { z } from "zod";
...
export { BROWSER_APPLICATIONS as DOCKER_BROWSER_ALIASES } from "@samiksha/core/node/desktop-runtime";
...
export function hasComputerIdentity(
  ...
  const labeledSpaceId = labels?.["samiksha.spaceId"] ?? labels?.["samiksha.workspaceId"];
  return labels?.["samiksha.botId"] === botId && labeledSpaceId === spaceId;
}
...
export function containerActionStep(
  ...
  if (action.kind === "key" || action.kind === "pointer" || action.kind === "clipboard") {
    ...
    argv = [
      "env",
      `DISPLAY=${display}`,
      ...(browserProfile ? [`SAMIKSHA_BROWSER_PROFILE=${browserProfile}`] : []),
      "xdg-open",
      target,
    ];
  } else {
    ...
    argv = [
      "env",
      `DISPLAY=${display}`,
      ...(browser && browserProfile ? [`SAMIKSHA_BROWSER_PROFILE=${browserProfile}`] : []),
      browser ? "samiksha-browser" : action.application,
...
export function workspaceTarget(relative: string) {
  return relative ? path.posix.join("/home/samiksha", relative) : "/home/samiksha";
}
