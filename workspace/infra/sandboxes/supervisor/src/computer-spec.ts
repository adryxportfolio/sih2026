...
import { MAX_DESKTOP_DISPLAY, screenPorts } from "@samiksha/core/node/desktop-runtime";
import type Docker from "dockerode";

export const COMPUTER_IMAGE = process.env.SAMIKSHA_COMPUTER_IMAGE ?? "samiksha/computer:local";
 ...
 * a host-wide memory and CPU event that takes every other bot and the Samiksha
...
export function computerResourceLimits() {
  const memoryBytes = parseMemoryBytes(
    ...
    process.env.SAMIKSHA_COMPUTER_MEMORY ?? DEFAULT_COMPUTER_MEMORY,
  );
  const nanoCpus = parseNanoCpus(
    "SAMIKSHA_COMPUTER_CPUS",
    process.env.SAMIKSHA_COMPUTER_CPUS ?? DEFAULT_COMPUTER_CPUS,
  );
  const pidsLimit = parsePidsLimit(
    "SAMIKSHA_COMPUTER_PIDS_LIMIT",
    process.env.SAMIKSHA_COMPUTER_PIDS_LIMIT ?? DEFAULT_COMPUTER_PIDS_LIMIT,
  );
  return {
...
export function containerCreateOptions(input: ComputerCreateInput) {
  ...
  return {
    ...
    Env: [
      ...
      "PIP_USER=1",
      ...(input.controlToken ? [`SAMIKSHA_COMPUTER_CONTROL_TOKEN=${input.controlToken}`] : []),
    ],
    Labels: {
      "samiksha.managed": "true",
      "samiksha.botId": input.botId,
      "samiksha.spaceId": input.spaceId,
    },
    ...
    HostConfig: {
      ...(input.homeVolume
        ...
        : { Binds: [`${input.homePath}:/home/samiksha`], Mounts: undefined }),
...
export function containerNameFor(botId: string) {
  return `samiksha-bot-${sanitizeIdentifier(botId)}`;
}
...
export function computerNetworkNameFor(botId: string) {
  ...
  return `samiksha-computer-${sanitizeIdentifier(botId).slice(0, 32)}-${hash}`;
}
...
export function computerNetworkNamesForCleanup(botId: string) {
  ...
  return [
    ...
    `samiksha-computer-${safe}`,
    `samiksha-computer-${safe.slice(0, 32)}-${digest.slice(0, 8)}`,
  ];
}
