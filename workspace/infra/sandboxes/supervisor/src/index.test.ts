  ...
  it("rejects a provision request whose identity headers do not match its body", async () => {
    const response = await supervisorApp.request("/computers", {
      ...
      headers: {
        authorization: `Bearer ${token}`,
        "content-type": "application/json",
        "x-samiksha-bot-id": "other-bot",
        "x-samiksha-space-id": "workspace",
      },
      body: JSON.stringify({
        botId: "bot",
        spaceId: "workspace",
...
describe("sandbox supervisor input containment", () => {
  ...
  it("accepts the legacy workspace label without weakening container identity", () => {
    expect(
      hasComputerIdentity({ "samiksha.botId": "bot", "samiksha.workspaceId": "space" }, "bot", "space"),
    ).toBe(true);
    expect(
      hasComputerIdentity(
        { "samiksha.botId": "bot", "samiksha.workspaceId": "other-space" },
        "bot",
        "space",
      ),
    ).toBe(false);
    expect(
      hasComputerIdentity(
        {
          "samiksha.botId": "bot",
          "samiksha.spaceId": "space",
          "samiksha.workspaceId": "other-space",
        },
        "bot",
        "space",
      ),
  ...
  it("routes Docker browser aliases through the safe wrapper on every display", () => {
    for (const application of DOCKER_BROWSER_ALIASES) {
      expect(
        ...
        argv: ["env", "DISPLAY=:2", "samiksha-browser", "https://example.com"],
      });
    }
  ...
  it("keeps the viewer read-only and uses a separate process for takeover control", () => {
    ...
    expect(interactiveScreenCommand(false, "lease-old")).toContain("SAMIKSHA_CONTROL_RELEASED");
  });
  ...
  it("resets stale managed screens without killing unrelated container jobs", () => {
    ...
    expect(command).toContain("for marker in /tmp/samiksha/browser-profile-*");
    expect(command).toContain("/tmp/samiksha/browser-pid-*");
  ...
  it("keeps a persistent independent profile per bot across display slots", () => {
    ...
    expect(command).not.toContain("/home/samiksha/.browser-profiles/chromium/.");
    expect(command).not.toContain(".samiksha-base-generation");
    expect(command).toContain("browser-pid-");
    expect(command).toContain("tr '\\0' '\\n' <\"/proc/$pid/cmdline\"");
    expect(browserProfilePathForScreen("../../writer")).toMatch(
      /^\/home\/samiksha\/\.browser-profiles\/chromium-bot-[0-9a-f]+$/,
    );
  });
  ...
  it("stops the released bot's browser without tearing down the primary desktop", () => {
    ...
    expect(primary).toContain("rm -f /tmp/samiksha/control-token-1");
