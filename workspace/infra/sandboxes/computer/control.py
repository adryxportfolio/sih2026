#!/usr/bin/env python3
"""Token-auth desktop control for the Samiksha supervisor."""
...
TOKEN = os.environ.get("SAMIKSHA_COMPUTER_CONTROL_TOKEN", "")
...
class NativeCapture:
    ...
    def __init__(self, display):
        library = ctypes.CDLL("/usr/local/lib/libsamiksha-xcapture.so")
        library.samiksha_xcapture_open.argtypes = [ctypes.c_char_p]
        library.samiksha_xcapture_open.restype = ctypes.c_void_p
        library.samiksha_xcapture_png.argtypes = [
            ctypes.c_void_p,
        ...
        library.samiksha_xcapture_png.restype = ctypes.c_int
        library.samiksha_xcapture_damage.argtypes = [
            ctypes.c_void_p,
            ctypes.POINTER(ctypes.c_int),
            ctypes.POINTER(ctypes.c_int),
            ctypes.POINTER(ctypes.c_int),
        ...
        library.samiksha_xcapture_damage.restype = ctypes.c_int
        library.samiksha_xinput_argv.argtypes = [ctypes.c_void_p, ctypes.c_int, ctypes.POINTER(ctypes.c_char_p)]
        library.samiksha_xinput_argv.restype = ctypes.c_int
        context = library.samiksha_xcapture_open(display.encode("utf-8"))
        if not context:
    ...
    def copy(self):
        ...
        if self.library.samiksha_xcapture_png(
        ...
        changed = self.library.samiksha_xcapture_damage(
    ...
    def act(self, argv):
        ...
        return self.library.samiksha_xinput_argv(self.context, len(argv), encoded)
...
def control_command_index(argv):
    ...
    return 3 if len(argv) > 2 and argv[2].startswith("SAMIKSHA_BROWSER_PROFILE=") else 2
...
def allowed_control_argv(argv, display):
    ...
    command = argv[index]
    if index == 3:
        profile = argv[2].removeprefix("SAMIKSHA_BROWSER_PROFILE=")
        if not re.fullmatch(r"/home/samiksha/\.browser-profiles/chromium-bot-[a-f0-9]{32}", profile):
            return False
        if command not in ("xdg-open", "samiksha-browser"):
            return False
    if command == "xdotool":
