...
loader = importlib.machinery.SourceFileLoader("page_browser", str(Path(__file__).with_name("samiksha-page-browser")))
...
class PageBrowserTest(unittest.TestCase):
    def test_closed_stdin_cancels_helper_without_a_browser(self):
        with subprocess.Popen([sys.executable, str(Path(__file__).with_name("samiksha-page-browser")), "snapshot", "{}"],
                              env={**os.environ, "SAMIKSHA_BROWSER_WATCH_STDIN": "1", "SAMIKSHA_CDP_PORT": "0"},
