...
const originalEnv = `SAMIKSHA_IMAGE_TAG=${oldTag}\nSAMIKSHA_IMAGE_TAG_PREVIOUS=v0.9.0\n`;
...
if (command === "docker") {
  ...
  call.imageTag = process.env.SAMIKSHA_IMAGE_TAG;
  log();
...
async function deployment(options: { failures: Failure[]; branch?: string; composePath?: string }) {
  const deployDir = await mkdtemp(path.join(os.tmpdir(), "samiksha-fork-recovery-"));
  ...
  const config = resolveUpdaterConfig({
    SAMIKSHA_DEPLOY_DIR: deployDir,
    SAMIKSHA_UPDATER_TOKEN: token,
    SAMIKSHA_COMPOSE_FILE: options.composePath,
  ...
  const compose = (revision: string) =>
    `services:\n  api:\n    image: example/app:\${SAMIKSHA_IMAGE_TAG}\n    env_file: ./service.env\n    command: ${revision}\n`;
  await Promise.all([
    mkdir(bin),
...
describe.skipIf(process.platform === "win32")("fork recovery through fake executables", () => {
  ...
  it("keeps the new checkout and image pin after a successful update", async () => {
    ...
    expect(await readFile(fixture.config.envFile, "utf8")).toContain(
      `SAMIKSHA_IMAGE_TAG=local-${targetCommit}\n`,
    );
  });
});
