...
import { RECREATED_SERVICES } from "@samiksha/core";
...
describe("the updater compose service", () => {
  ...
  it("is bind-mounted at the same path it has on the host", () => {
    const mount = (updater.volumes ?? []).find((volume) => volume.includes("SAMIKSHA_DEPLOY_DIR"));
    // biome-ignore lint/suspicious/noTemplateCurlyInString: this is the literal Compose expression
    const deployDir = "${SAMIKSHA_DEPLOY_DIR:-/srv/samiksha}";
    ...
    expect(updater.environment?.SAMIKSHA_DEPLOY_DIR).toBe(deployDir);
  ...
  it("pins its own image tag separately from the application image", () => {
    expect(updater.image).toContain("SAMIKSHA_UPDATER_IMAGE_TAG");
    for (const service of RECREATED_SERVICES) {
      expect(compose.services[service]?.image).toContain("SAMIKSHA_IMAGE_TAG");
    }
  });

  it("uses the official registry namespace and digest-pins third-party runtime images", () => {
    expect(updater.image).toContain("ghcr.io/elie222/samiksha/updater");
    expect(compose.services.api?.image).toContain("ghcr.io/elie222/samiksha/app");
  ...
  it("lets a deployment name its own Compose files and extra services", () => {
    ...
    expect(updater.environment?.SAMIKSHA_COMPOSE_FILE).toBe(
      interpolated("SAMIKSHA_COMPOSE_FILE", "infra/compose/docker-compose.prod.yml"),
    );
    expect(updater.environment?.SAMIKSHA_UPDATE_SERVICES).toBe(
      interpolated("SAMIKSHA_UPDATE_SERVICES"),
    );
  ...
  it("injects the actual Compose project name into the updater container", () => {
    // biome-ignore lint/suspicious/noTemplateCurlyInString: this is the literal Compose expression
    expect(updater.environment?.COMPOSE_PROJECT_NAME).toBe("${COMPOSE_PROJECT_NAME:-samiksha-prod}");
  });

  it("does not load the application env_file into the root-equivalent process", () => {
    expect(updater.env_file).toBeUndefined();
    // biome-ignore lint/suspicious/noTemplateCurlyInString: this is the literal Compose expression
    expect(updater.environment?.SAMIKSHA_UPDATER_TOKEN).toBe("${SAMIKSHA_UPDATER_TOKEN:-}");
  });
  ...
  it("does not let the api container reach the Docker socket to update itself", () => {
    ...
    expect(compose.services.api?.environment?.SAMIKSHA_UPDATER_URL).toBe("http://updater:7092");
  });
