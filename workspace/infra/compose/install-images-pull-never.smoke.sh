...
[[ "$offline_out" == *"Samiksha is starting"* ]] || fail "--offline did not start"
...
[[ "$old_out" != *"Samiksha setup failed:"* ]] || fail "old Compose --offline should not hard-fail: $old_out"
[[ "$old_out" == *"Samiksha is starting"* ]] || fail "old Compose --offline should continue: $old_out"
