...
PROJECT_DIR="${SAMIKSHA_DEPLOY_DIR:-/srv/samiksha}"
[[ "${PROJECT_DIR}" == /* ]] || { echo "SAMIKSHA_DEPLOY_DIR must be an absolute path" >&2; exit 1; }
COMPOSE_FILE="${PROJECT_DIR}/infra/compose/docker-compose.prod.yml"
ENV_FILE="${PROJECT_DIR}/.env"
BACKUP_ROOT="/var/backups/samiksha"
...
"${compose[@]}" exec -T postgres pg_restore --list \
  < "${SNAPSHOT_DIR}/samiksha.dump" >/dev/null
tar -tzf "${SNAPSHOT_DIR}/appdata.tgz" >/dev/null

sha256sum "${SNAPSHOT_DIR}/samiksha.dump" "${SNAPSHOT_DIR}/appdata.tgz" \
...
echo "Verified Samiksha backup written to ${SNAPSHOT_DIR}"
