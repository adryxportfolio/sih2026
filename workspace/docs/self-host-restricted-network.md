...
| Cannot fetch Compose files | `SAMIKSHA_DOWNLOAD_BASE`, `--local`, or `SAMIKSHA_DOWNLOAD_SKIP_EXISTING=1` |
| Cannot pull app or computer images | `SAMIKSHA_IMAGE`, `SAMIKSHA_IMAGE_TAG`, `SAMIKSHA_COMPUTER_IMAGE`, `SAMIKSHA_COMPUTER_IMAGE_TAG` |
...
```bash
export SAMIKSHA_INSTALLER_URL=https://example.com/mirror/samiksha/infra/compose/install-images.sh
export SAMIKSHA_DOWNLOAD_BASE=https://example.com/mirror/samiksha/infra/compose
mkdir -p samiksha && cd samiksha &&
curl -fsSL -o install-images.sh "${SAMIKSHA_INSTALLER_URL}" &&
bash install-images.sh --prepare-only
```

`SAMIKSHA_INSTALLER_URL` selects the script for this curl command; `SAMIKSHA_DOWNLOAD_BASE` selects
...
SAMIKSHA_DOWNLOAD_SKIP_EXISTING=1 bash install-images.sh --prepare-only
```
...
```env
SAMIKSHA_IMAGE=registry.example.com/mirror/samiksha/app
SAMIKSHA_IMAGE_TAG=edge
SAMIKSHA_COMPUTER_IMAGE=registry.example.com/mirror/samiksha/computer
SAMIKSHA_COMPUTER_IMAGE_TAG=edge
